...
export function EditEventDialog({
  ...
  React.useEffect(() => {
    if (open && event) {
      ...
      setSelectedSkills(event.skill_ids ?? [])
      setTagSearch("")
      setTagList([])
      setTagHasMore(true)
  ...
  const mergedTagList = React.useMemo(() => {
    const resultIds = new Set(tagList.map((tag) => tag.id))
  ...
  const fetchTags = React.useCallback(
    async ({ reset }: { reset: boolean }) => {
      ...
      let query = supabase.from("event_tags").select("id, name, color")
        .eq("org_id", organizationId)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await supabase.from("event_tag_links").delete().eq("event_id", event.id)
    await supabase.from("event_skill_links").delete().eq("event_id", event.id)
        ...
        <div className="relative flex h-full flex-col">
          ...
          <SheetFooter className="border-t px-6 py-4">
            <div className="flex w-full flex-col gap-3 sm:flex-row sm:justify-end">
              ...
              <Button form="edit-event-form" type="submit" disabled={loading}>
