...
import type { TaskWithEvent } from "@/components/tasks/all-tasks"
import type {
  EventOrgMember,
  EventSkill,
...
export default async function MyTasksPage({ params }: MyTasksPageProps) {
  ...
  const currentMemberId =
    allMembers.find((member) => member.user_id === user.id)?.id ?? null
  ...
  const tasks = (tasksResult ?? []).map((task: TaskRow) => {
  ...
  return (
    <div className="flex flex-1 flex-col">
      <DashboardHeader title="My tasks" sectionHref={`/${orgSlug}/tasks`} sectionLabel="Tasks" />
